...
//Based code for system
int WIDTH = 64, HEIGHT = 32;
int scalefactor = 10; //If you change the scale factor, remember to change the textures as well

//Chip 8 System Globals.
...
void Chip8DisplayOut(SDL_Renderer *renderer);
void Chip8Init();
void Chip8CPU(uint16_t opcode);
...
int main(int argc, char *argv[]) 
{
    ...
    SDL_Window *window = SDL_CreateWindow("Chip-8 Emulator", SDL_WINDOWPOS_UNDEFINED,SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_ALLOW_HIGHDPI);
    SDL_Renderer *renderer = SDL_CreateRenderer(window, -1, 0);
    
    //Run code in loop,.
    while (true) {
        //Fetch Opcode
        uint16_t Opcode = Chip8Memory[PC] << 8 | Chip8Memory[PC + 1]; //Opcode is 2 bytes long, so we need to combine the two bytes into one 16 bit number.
        ...
        if (DisplayUpdate) {
            Chip8DisplayOut(renderer);
        }

        Chip8UpdateTimers();
        PC += 2; //Increment PC by 2, since each opcode is 2 bytes long.
...
void Chip8Init() {
    ...
    for (int x = 0; x < 64; x++) {
        for (int y = 0; y < 32; y++) {
            Chip8Display[x][y] = 0; //Set each pixel to off.
        }
    }

    //Load font set into memory location 0x50 to 0x9F, (Location 80 and 159)
    for (int f = 80; f < 160; f++) {
        Chip8Memory[f] = FONT[f-80];
    }

    //Get Rom File from User
    cout << "Please enter the file name of your ROM file." << endl << "Please include the .ch8 at the end! " << endl;
    cin >> ROMName;

    ifstream file(ROMName, std::ios::binary);

    if (!file.is_open()) {
        cout << "Unable to open ROM file, exiting...." << endl;
        exit(EXIT_FAILURE);
    }    

    file.seekg(0, ios::end);//Use to find length of the ROM, basically
    ...
    if (ROMSIZE > 3586) {
        cout << "ROM File too large to fit in memory, exiting...." << endl;
        file.close();
        exit(EXIT_FAILURE);
    }

    //Load rom into memory
    file.read(reinterpret_cast<char*>(Chip8Memory) + 0x200, ROMSIZE);
...
void Chip8UpdateTimers() {
    if (DelayTimer > 0) { //Even though timers are supposed to update asynchonously, I don't know how to use multithreading.
        DelayTimer--;
    }
    if (SoundTimer > 0) {
        SoundTimer--;
...
void Chip8CPU(uint16_t opcode) {
    switch (opcode & 0xF000) //Check the value of the first 4 bits
    {
        ...
        case 0x1000: //Jump
            PC = opcode & 0x0FFF; //Since our system increments PC anyways, we need to subtract by two.
            PC -= 2;
            break;
        ...
        case 0x8000: //LOTS of Math operations
            switch (opcode & 0x000F) //Check the value of the last 4 bits
            {
                ...
                {
                    uint8_t XAddval = V[(opcode & 0x0F00) >> 8];
                    uint8_t YAddval = V[(opcode & 0x00F0) >> 4];
        ...
        case 0xD000: //Draw sprite 
        {
            ...
            uint8_t Spritex = V[(opcode & 0x0F00) >> 8] % 64;
            uint8_t Spritey = V[(opcode & 0x00F0) >> 4] % 32; //Get values from the registers
            ...
            for (int ylen = 0; ylen < spriteheight; ylen++) {
                ...
                spritepixel = Chip8Memory[I + ylen]; //The current pixel being drawn, given to us from the I value and the sprite height.
                ...
                for (int xlen = 0; xlen < 8; xlen++) { //X val is preset always to 8, since the sprite is 8 pixels wide.
                    ...
                    if ((spritepixel & (0x80 >> xlen)) != 0) {
                        ...
                        if (Chip8Display[(Spritex + xlen)][(Spritey + ylen)] == 1) { 
                            //If there is a colision, where a pixel is alreay on, set it to zero and set the colision flag to 1.
                            V[15] = 1; //Set colision value equal to 1
                        }

                        Chip8Display[(Spritex + xlen)][(Spritey + ylen)] ^= 1; //Exclusive OR the pixel
                    }
                }
            }
        ...
        case 0xE000: //Skip next instruction if key is pressed or not pressed
            switch (opcode & 0x00FF)
            {
                case 0x009E: //Skip instruction if key is pressed
                    ...
                    if (Chip8KeyPad[V[(opcode & 0x0F00) >> 8]] != 0) {
                        PC += 2;
                    }
                    break;
        ...
        case 0xF000: //Lots of loading operations
            switch (opcode & 0x00FF) 
            {
                ...
                case 0x000A: //Wait for keypress then store in Vx 
                {
                    ...
                    PC -= 2; //Since we are waiting for a keypress, we need to decrement the PC by 2. 
                ...
                case 0x0033: //Store the decimal representation of Vx in memory locations I, I+1, and I+2
                    Chip8Memory[I] = V[(opcode & 0x0F00) >> 8] / 100;
                    Chip8Memory[I + 1] = (V[(opcode & 0x0F00) >> 8] / 10) % 10;
                    Chip8Memory[I + 2] = V[(opcode & 0x0F00) >> 8] % 10;
                    break;
